...
class ShippingService {
	...
	private async createShipmentReal(data: CreateShipmentRequest): Promise<Shipment> {
		if (!browser) {
			throw new ShippingError('Cannot create shipment on server side');
		}
	...
	private async updateShipmentStatusReal(
		...
		if (!browser) {
			throw new ShippingError('Cannot update shipment on server side');
		}
	...
	private async getShipmentByIdReal(id: string): Promise<Shipment> {
		if (!browser) {
			throw new ShippingError('Cannot get shipment on server side');
		}
